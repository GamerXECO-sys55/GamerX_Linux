...
LOG_FILE="/tmp/gaming_profile_install.log"
exec 1> >(tee -a "$LOG_FILE")
exec 2> >(tee -a "$LOG_FILE" >&2)

echo "$(date '+%Y-%m-%d %H:%M:%S') [INFO] Gaming Profile Installer Started" | tee -a "$LOG_FILE"

echo -e "\e[1;32mGaming Profile Installer Triggered\e[0m"
...
if [ -f "$PACKAGES_FILE" ]; then
    echo "$(date '+%Y-%m-%d %H:%M:%S') [INFO] Installing Gaming packages from $PACKAGES_FILE" | tee -a "$LOG_FILE"
    ...
    echo "$(date '+%Y-%m-%d %H:%M:%S') [INFO] Gaming packages installation completed!" | tee -a "$LOG_FILE"
else
    echo "$(date '+%Y-%m-%d %H:%M:%S') [ERROR] Package list not found at $PACKAGES_FILE" | tee -a "$LOG_FILE"
fi

echo "$(date '+%Y-%m-%d %H:%M:%S') [INFO] Gaming Profile Installer Completed" | tee -a "$LOG_FILE" 